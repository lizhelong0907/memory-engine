...
export class KnowledgeExtractor {
 ...
 // 从用户消息提取知识
 async extractFromUserMessage(content: string): Promise<KnowledgeExtractionResult | null> {
 ...
 // 根据标签确定知识类型
 private determineKnowledgeType(tags: string[], content: string): { knowledgeType: MemoryType; importance: number } {
 // 检查标签中是否包含特定类型
 ...
 // 从AI消息提取知识
 async extractFromAIMessage(content: string, userMessage?: string): Promise<KnowledgeExtractionResult | null> {
 const trimmed = content.trim();
 ...
 // 从对话对提取知识
 async extractFromConversationPair(
 userMessage: string,
 aiMessage: string
 ...
 // 辅助方法：检测技术解释
 private isTechnicalExplanation(content: string): boolean {
 ...
 // 辅助方法：检测问题解决
 private isProblemSolving(userMsg: string, aiMsg: string): boolean {
