...
import { AfterTurnParams, AfterTurnResult, AssembleParams, CompactParams, AssembleResult } from './types';
...
let enginePromise: Promise<MemoryEngine> | null = null;

class MemoryEngineContextEngine {
 private config: Record<string, any> = {};
 private embeddingCache = new Map<string, number[]>();
 private maxEmbeddingCacheSize =1000;
 private recallTopK =5;
 private memoryFetchLimit =100;
 private similarityThreshold =0.3;
 private semanticThreshold =0.35;
 private storeMinLength =6;
 private dedupSimilarityThreshold =0.92;
 private stats = {
 searches:0,
 hits:0,
 totalSearchMs:0,
 ...
 private cleanupMaxAgeDays =90;
 private cleanupMinImportance =0.4;
 ...
 private resolveBoolean(value: any, fallback: boolean): boolean {
 ...
 private resolveNumber(value: any, fallback: number): number {
 if (value === undefined || value === null || value === '') return fallback;
 const n = Number(value);
 return Number.isFinite(n) ? n : fallback;
 }

 private async getCachedEmbedding(cacheKey: string, text: string, embeddingModel: any): Promise<number[]> {
 ...
 enginePromise = createMemoryEngine({
 dbPath,
 ...
 private processUntrustedMetadata(content: string): string {
 if (!content) return '';
 ...
 // 每轮对话后处理
 async afterTurn(params: AfterTurnParams): Promise<AfterTurnResult> {
 ...
 if (!this.shouldStoreMemory(content)) continue;
 ...
 try {
 const extractor = (engine as any).extractor;
 ...
 if (archived >0) debugLog('🧹 Archived memories:', archived);
 } catch (e) {
 ...
 const lastUser = params.messages.filter((m: any) => m?.role === 'user').pop();
 if (lastUser) {
 ...
 messages = compressed.messages.map((m: any) => ({ role: m.role, content: m.content }));
 ...
 const allContent = messages.map((m: any) => m.content || '').join('') + system;
 ...
 private async shouldSearchMemories(content: string): Promise<boolean> {
 ...
 private shouldStoreMemory(content: string): boolean {
 const text = (content || '').trim();

 //过滤寒暄/确认/无意义
 if (/^(你好|您好|嗨|hello|hi|hey|早上好|下午好|晚上好|再见|拜拜)$/.test(text)) return false;
 if (/^(好的|收到|明白|ok|okay|thanks|谢谢|知道了|嗯+|啊+|哦+)$/.test(text)) return false;

 // 简单敏感信息过滤（可扩展）
 if (/\b\d{15,}\b/.test(text)) return false; // 超长数字

 // 偏好类短句直接放行
 if (/(喜欢|爱好|偏好|讨厌)/.test(text)) return true;
 ...
 if (cjkLen < this.storeMinLength) return false;
 } else {
 // 英文：按单词数
 const wordCount = text.split(/\s+/).filter(Boolean).length;
 if (wordCount < Math.max(2, Math.floor(this.storeMinLength /2))) return false;
 }

 return true;
 }

 private async isNearDuplicate(engine: MemoryEngine, embeddingModel: any, embedding: number[]): Promise<boolean> {
 ...
 if (similarity >= this.dedupSimilarityThreshold) return true;
 }
 ...
 // 快速规则检查
 private quickRuleCheck(content: string): boolean | null {
 const compact = content.toLowerCase().replace(/\s+/g, '');

 //纯寒暄/确认/感谢
 if (/^(你好|您好|嗨|hello|hi|hey|早上好|下午好|晚上好|再见|拜拜)$/.test(compact)) return false;
 if (/^(好的|收到|明白|ok|okay|thanks|谢谢|知道了|嗯+|啊+|哦+)$/.test(compact)) return false;
 ...
 //语义检查（使用 embedding）
 private async semanticCheck(content: string): Promise<boolean> {
 try {
 // 若无问句/回忆意图，直接不搜
 ...
 private async searchMemoriesForContext(engine: MemoryEngine, userContent: string): Promise<{ systemPromptAddition: string; activeRecallPrompts: string[] }> {
 ...
 private keywordRecall(memories: any[], userContent: string): Set<string> {
 ...
 if (item.keywords.some(k => text.includes(k))) matchedTags.add(item.tag);
 }
 ...
 if (this.simpleOverlap(contentLower, text)) hits.add(memory.id);
 }
 }
 ...
 private simpleOverlap(a: string, b: string): boolean {
 const tokens = b.split(/\s+/).filter(Boolean).slice(0,6);
 return tokens.some(t => t.length >=2 && a.includes(t));
 }

 private groupByType(memories: any[]): Record<string, any[]> {
 ...
 private buildMemoryText(grouped: Record<string, any[]>): string {
 ...
 if (lines.length >= this.recallTopK) break;
 }
 if (lines.length >= this.recallTopK) break;
 }
 return lines.join('\n');
 ...
 private recordSearchStats(start: number, hitCount: number) {
 const cost = Date.now() - start;
 this.stats.lastSearchMs = cost;
 this.stats.totalSearchMs += cost;
 if (hitCount >0) this.stats.hits +=1;
 }

 // 压缩上下文（OpenClaw hook）
 async compact(_params: CompactParams): Promise<void> {
