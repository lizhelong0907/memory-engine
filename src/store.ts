...
import { MemoryRecord, MemoryCreateInput, MemoryStatus, SearchOptions } from './types';
...
export class Store {
 ...
 embedding: (input as any).embedding
 } as any;
 ...
 const params: any = { status: options.status || 'active' };
 ...
 const updated = { ...existing, ...updates, updatedAt: Date.now() } as any;
 ...
 // 获取所有记忆
 getAllMemories(limit: number =50): MemoryRecord[] {
 return this.search({ limit });
 }

 // 获取统计信息
 getStats(): any {
 if (!this.db) return { totalCount:0, activeCount:0, byType:{} };
 ...
 byType: byType.reduce((acc: any, row: any) => { acc[row.type] = row.count; return acc; }, {})
 };
 }
